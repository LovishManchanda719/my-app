...
import { AlertTriangle, Droplet, Activity, Thermometer, Calendar, TrendingUp, Settings, AlertOctagon, PhoneCall } from 'lucide-react';
...
const generateMockData = () => {
  ...
  return nodes.map(node => ({
    ...
    leakStatus: Math.random() > 0.8 ? 'Critical' : Math.random() > 0.6 ? 'Warning' : 'Normal',
...
const LeakAlert = ({ status }) => {
  const alertType = status === 'Critical' ? 'destructive' : status === 'Warning' ? 'warning' : 'success';
  const title = status === 'Normal' ? 'No Leaks Detected' : `${status} Leak Detected`;
  ...
  <Card>
    ...
    <CardContent>
      ...
      <p className="mt-2 text-sm text-gray-500">
        {data.maintenanceScore > 80 ? 'System is in good condition' :
         data.maintenanceScore > 50 ? 'Maintenance may be required soon' :
         'Urgent maintenance required'}
      </p>
      {data.maintenanceScore <= 50 && (
        <Button className="mt-2 w-full" onClick={() => alert(`Maintenance request escalated for ${data.name}`)}>
    ...
    <CardContent>
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={data}>
          ...
          <YAxis />
          <Tooltip />
          <Line type="monotone" dataKey="consumption" stroke="#3b82f6" name="Actual Consumption" />
          <Line type="monotone" dataKey="forecast" stroke="#10b981" name="Forecasted Consumption" />
        </LineChart>
      </ResponsiveContainer>
    </CardContent>
  </Card>
...
const WaterScarcityPrediction = ({ data }) => {
  const scarcityRisk = data.reduce((acc, node) => acc + node.waterLevel, 0) / data.length;
  const riskLevel = scarcityRisk < 30 ? 'High' : scarcityRisk < 60 ? 'Medium' : 'Low';
    ...
    <Card>
      ...
      <CardContent>
        ...
        <p className="mt-2 text-sm">
          Based on current water levels and demand forecasts, the risk of water scarcity in the near future is {riskLevel.toLowerCase()}.
        </p>
        {riskLevel !== 'Low' && (
          <Button className="mt-2 w-full" onClick={() => alert('Water conservation measures activated')}>
            <AlertOctagon className="mr-2 h-4 w-4" /> Activate Conservation Measures
          </Button>
        )}
      </CardContent>
    </Card>
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className={supplyDemandRatio < 80 ? 'text-red-600' : 'text-green-600'}>
              {supplyDemandRatio.toFixed(1)}%
            </span>
          </div>
        </div>
        {supplyDemandRatio < 80 && (
          <Button className="mt-4 w-full" onClick={() => alert('Drought management plan activated')}>
            <AlertOctagon className="mr-2 h-4 w-4" /> Activate Drought Management Plan
          </Button>
        )}
      </CardContent>
    </Card>
...
const WaterManagementDashboard = () => {
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto">
      <h1 className="text-3xl font-bold mb-4">IoT Water Management Dashboard</h1>
              ...
              <Card key={index}>
                ...
                <CardContent>
                  ...
                  <div className={`mt-2 p-2 rounded-md text-center ${
                    node.leakStatus === 'Critical' ? 'bg-red-100 text-red-800' :
                    node.leakStatus === 'Warning' ? 'bg-yellow-100 text-yellow-800' :
